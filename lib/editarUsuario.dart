...
import 'package:consumir/listarUsuarios.dart';
...
class _EditarState extends State<Editar> {
  ...
  // ignore: prefer_final_fields. contraseña no sea vea y al darle click se vea
  ...
  @override
  void initState() {// cuando se carga la aplicacion , inicia lo que hay dentro la  funcion get la llama
    super.initState();
    nomUsuario.text = widget.texto;// widget trae lo de la lista cuando se modifica
    getUsuarios();
  }

  Future<void> getUsuarios() async {//metodo get
    final response =
        await http.get(Uri.parse('https://api-twbs.onrender.com/api/usuario'));
    if (response.statusCode == 200) {
      Map<String, dynamic> decodedData = json.decode(response.body);// map es un tipo de lista con el nombre usuarios
      setState(() {
  ...
  Future<void> _modificarUsuario(String rol, String nom, String ape, String ema,
    ...
    if (_formKey.currentState?.validate() ?? false) {
      if (nomUsuario.text.length < 5 ||
        ...
        if (response.statusCode == 200) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Empleado modificado...')),
          );
                    ...
                    Padding(
                      ...
                      child: TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          helperText: 'Escoja un rol.',
                          counterText: '${rol.text.length}/30',
                        ),
                        validator: (value) {
                          if (value == null || value.isEmpty) {
                            value = widget.texto;
